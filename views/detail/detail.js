...
const saleCount = document.querySelector('.salesCount');
...
plusBtn.addEventListener('click', () => {
  saleCount.innetText + 1;
  if (parseInt(saleCount.innerText) >= 10) {
...
minusBtn.addEventListener('click', () => {
  if (parseInt(saleCount.innerText) <= 1) {
    alert('최소 구매 수량은 1개 입니다.');
    return;
  } else {
    saleCount.innerText = parseInt(saleCount.innerText) - 1;
  }
});
...
async function getDataFromApi() {
  ...
  const { name, category, price, imageUrl, content } = res.data;
